  ...
  async.parallel({
		...
		books: function(callback) {
			Book.find({},'title').exec(callback);
				...
				// Success.
				res.render('bookinstance_form', { title: 'Update Book instance', book_list: results.books, bookinstance: results.book_instance, selected_book: results.book_instance.book._id });
		});
};
	...
	(req, res, next) => {
			...
			if (!errors.isEmpty()) {
					...
					async.parallel({
							...
							res.render('bookinstance_form', { title: 'Update Book instance', books: results.books, errors: errors.array() });
					});
					return;
			}
			else {
